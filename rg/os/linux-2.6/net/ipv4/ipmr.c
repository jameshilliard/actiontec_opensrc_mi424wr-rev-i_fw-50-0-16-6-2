 ...
 *
 *	Version: $Id: ipmr.c,v 1.5 2011/12/06 21:28:32 jacoba Exp $
 *
 *	Fixes:
...
static void (*mcast_rx_hook)(struct sk_buff *skb);

void impr_set_mcast_rx_hook(void (*hook)(struct sk_buff *skb))
{
    mcast_rx_hook = hook; 
}

EXPORT_SYMBOL(impr_set_mcast_rx_hook);
#endif
...
static int vif_add(struct vifctl *vifc, int mrtsock)
{
	...
	case 0:
		if (vifc->vifc_flags == VIFF_USE_IFINDEX) {
			dev = dev_get_by_index(vifc->vifc_lcl_ifindex);
