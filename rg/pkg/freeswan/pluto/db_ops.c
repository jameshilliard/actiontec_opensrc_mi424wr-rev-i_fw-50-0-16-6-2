 ...
 * 
 * $Id: db_ops.c,v 1.2 2012/01/24 19:47:58 guyn Exp $
 *
...
db_trans_expand(struct db_context *ctx, int delta_trans)
{
	...
	ctx->trans_cur = (struct db_trans *)((char *)(ctx->trans_cur) + offset);
	/* update elem count */
	ctx->max_trans = max_trans;
...
db_attrs_expand(struct db_context *ctx, int delta_attrs)
{
	...
	ctx->attrs0 = (struct db_attr *)((char *)ctx->attrs0 + offset);
	ctx->attrs_cur = (struct db_attr *)((char *)ctx->attrs_cur + offset);
	/* for each transform, rewrite attrs pointer by offsetting it */
	for (t=ctx->prop.trans, ti=0; ti < ctx->prop.trans_cnt; t++, ti++) {
	    t->attrs = (struct db_attr *)((char *)t->attrs + offset);
	}
	/* update elem count */
	ctx->max_attrs = max_attrs;
