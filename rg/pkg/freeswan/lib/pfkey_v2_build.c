...
char pfkey_v2_build_c_version[] = "$Id: pfkey_v2_build.c,v 1.12 2012/01/24 19:47:58 guyn Exp $";
...
static int anti_replay_enabled = 1;

void pfkey_anti_replay_enabled_set(int is_enabled)
{
    anti_replay_enabled = is_enabled;
}
	       ...
	       uint32_t			flags)
{
	...
	pfkey_sa->sadb_sa_replay = anti_replay_enabled ? replay_window : 0;
		    ...
		    uint32_t		log_all)
{
	...
	DEBUGGING(
		"pfkey_x_debug_build: "
		"tunnel=%x netlink=%x xform=%x eroute=%x spi=%x radij=%x esp=%x ah=%x rcv=%x pfkey=%x ipcomp=%x verbose=%x reject=%x log_all=%x\n",
		tunnel, netlink, xform, eroute, spi, radij, esp, ah, rcv, pfkey, ipcomp, verbose, reject, log_all);
	...
	pfkey_x_debug->sadb_x_debug_reject = reject;
	pfkey_x_debug->sadb_x_debug_log_all = log_all;
	for(i=0; i<4; i++) {
		pfkey_x_debug->aligned[i] = 0;
	}
	...
	for(ext = 1; ext <= SADB_EXT_MAX; ext++) {
		...
		if(extensions[ext]) {    
			...
			memcpy(pfkey_ext,
			       extensions[ext],
			       (extensions[ext])->sadb_ext_len * IPSEC_PFKEYv2_ALIGN);
			pfkey_ext = (struct sadb_ext *)(((char*)pfkey_ext) + (extensions[ext])->sadb_ext_len * IPSEC_PFKEYv2_ALIGN);
			/* Mark that we have seen this extension and remember the header location */
			extensions_seen |= ( 1ULL << ext );
		}
	}
