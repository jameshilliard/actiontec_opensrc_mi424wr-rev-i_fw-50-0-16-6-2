...
static void atheros_ar5xxx_add_ordered(wl_net_type_t *vap_order)
{
	...
	"ath%d", 1, vap_order);
    if (token_get("ACTION_TEC_DIAGNOSTICS"))
    wireless_vaps_add("ath1", config2type(ath_first_vap_types),
	"ath%d", 1, vap_order);
...
static void marvell_8xxx_add(int dev_idx, int num_vaps)
{
    ...
    wireless_phy_dev_add(DEV_IF_MV8XXX_RADIO, devname, -1, 0);

    /* Add Virtual Access Points */
    for (i = 0; i < num_vaps; i++)
    {
	str_printf(&vapname, "%sap%d", devname, i);
	wireless_vap_add(DEV_IF_MV8XXX_VAP, devname, vapname, -1, 1, 1);
    }
}
...
void hardware_features(void)
{
	...
	/* Ethernet drivers */
		...
		DEV_IF_NET_INT, token_get("CONFIG_RG_VZ_BHR") ? 5 : 4);
#if 0
        ...
        if (token_get("CONFIG_HW_MOCA"))
	{
	    ...
	    token_set_y("CONFIG_ENTROPIC_EN2710_MII");
        /* RevI ethernet moca union */
        token_set_y("CONFIG_RG_MOCA_NO_UNDERLYING");

	    dev_add("clink1", DEV_IF_MOCA_MII, DEV_IF_NET_EXT);
	    dev_add("clink0", DEV_IF_MOCA_MII, DEV_IF_NET_INT);
