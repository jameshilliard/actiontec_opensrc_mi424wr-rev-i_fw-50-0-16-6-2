...
static inline int mv_eth_skb_mh_add(struct sk_buff *skb, u16 mh)
{
	...
	if (skb_headroom(skb) < MV_ETH_MH_SIZE) {
        ...
        printk(KERN_ERR "adding mh to skb 0x%x\n", mh);
...
int mv_eth_ctrl_rxq_size_set(int port, int rxq, int value)
{
	...
	if (rxq >= CONFIG_MV_ETH_RXQ) {
		printk(KERN_INFO "port (%d) queue (%d) doens not exist %s\n" , port, rxq, __func__);
		return -EINVAL;
	}
...
static MV_U32 aei_gpio_led_grp, aei_gpio_led_mask, aei_gpio_led_val;
static int aei_led_active;

#define aei_led_off() aei_led_controll(0)
#define aei_led_on() aei_led_controll(1)
static void aei_led_controll(int is_on)
{
    mvGppValueSet(aei_gpio_led_grp, aei_gpio_led_mask,
	~(is_on ? aei_gpio_led_val : 0));
}

static void aei_led_init(int is_link, int is_10, int is_100)
{
    MV_32 gpio_10 = mvBoarGpioPinNumGet(BOARD_GPP_GBE_10, 0);
    MV_32 gpio_100 = mvBoarGpioPinNumGet(BOARD_GPP_GBE_100, 0);
    MV_U32 ethConfig = mvBoardEthComplexConfigGet();

    if (gpio_10 < 0 || gpio_100 < 0)
    {
	aei_gpio_led_grp = aei_gpio_led_mask = aei_gpio_led_val = 0;
        aei_led_active = 0;
	return;
    }

    /* Both 10 and 100 LEDs MUST be in the same MPP group */
    aei_gpio_led_grp = gpio_10 >> 5;

    aei_gpio_led_mask = (1 << (gpio_10 & 0x1F)) | (1 << (gpio_100 & 0x1F));
    ...
    {
        aei_led_active = 1;
        aei_gpio_led_val = (is_link && is_10 ? 1 << (gpio_10 & 0x1F) : 0) |
            (is_link && is_100 ? 1 << (gpio_100 & 0x1F) : 0);
    }
    
    printk("GBE LED: link:%d, 10:%d, 100:%d, grp/mask/val:%x/%x/%x\n",
	is_link, is_10, is_100,
	aei_gpio_led_grp, aei_gpio_led_mask, aei_gpio_led_val);

    mvGppTypeSet(aei_gpio_led_grp, aei_gpio_led_mask, 0);
    mvGppValueSet(aei_gpio_led_grp, aei_gpio_led_mask, ~aei_gpio_led_val);    
}
...
static void aei_led_set(int port)
{
    ...
    aei_led_init(
	status & NETA_GMAC_LINK_UP_MASK,
	!(status & NETA_GMAC_SPEED_100_MASK),
	status & (NETA_GMAC_SPEED_100_MASK | NETA_GMAC_SPEED_1000_MASK));
}
...
int mv_eth_tx_txq(int priority, int is_hwq)
{
    return (priority & 0x6) + ((is_hwq) ? 1 : 0); 
}
EXPORT_SYMBOL(mv_eth_tx_txq);
...
static inline int mv_eth_tx_policy(struct eth_port *pp, struct sk_buff *skb)
{
	...
	return txq;
#else
	return mv_eth_tx_txq(skb->priority, 0);
#endif       	
}

#ifdef CONFIG_NET_SKB_RECYCLE
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 24)
static inline int mv_eth_skb_recycle_check(struct sk_buff *skb, int skb_size)
{
	...
	if (skb_shared(skb) || skb_cloned(skb) || skb_shinfo(skb)->frag_list)
		return 0;
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 24)
	if (mv_eth_skb_recycle_check(skb, pool->pkt_size)) {
...
static inline MV_U32 mv_eth_skb_tx_csum(struct eth_port *pp, struct sk_buff *skb)
{
	...
	if (skb->ip_summed == CHECKSUM_PARTIAL) {
		...
		if (skb->protocol == htons(ETH_P_IP)) {
			...
			ip_hdr_len = skb->h.raw - skb->nh.raw;
		...
		ip_offset = skb->nh.raw - skb->data;

		return mvNetaTxqDescCsum(ip_offset, skb->protocol, ip_hdr_len, l4_proto);
	}
...
static inline int mv_eth_tso_validate(struct sk_buff *skb, struct net_device *dev)
{
	...
	if ((htons(ETH_P_IP) != skb->protocol) || ((skb->nh.iph)->protocol != IPPROTO_TCP) || (skb->h.th == NULL)) {
					     ...
					     MV_U32 tcp_seq, MV_U16 ip_id, int left_len)
{
	...
	struct tcphdr *tcph;
	MV_U8 *data, *mac;
	int mac_hdr_len = (u32)(skb->nh.raw - skb->data);
	...
	tcph = (struct tcphdr *)(mac + (u32)(skb->h.raw - skb->data));
...
*/
	...
	total_len = skb->len;
	hdr_len = ((u32)((u32)(skb->h.th) - (u32)skb->data) + (u32)((skb->h.th)->doff * 4));
...
static int mv_eth_pool_destroy(int pool)
{
	...
	if (ppool->bm_pool)
		mvOsIoUncachedFree(NULL, (ppool->capacity * sizeof(MV_U32)), ppool->phys_addr, ppool->bm_pool, 0);
...
#ifdef CONFIG_MV_ETH_BM_CPU
	bm_pool->bm_pool = mv_eth_bm_pool_create(pool, capacity, &bm_pool->phys_addr);
...
/***********************************************************************************************/
int mv_eth_poll(struct net_device *dev, int *in_budget)
{
	int rx_done = 0;
	MV_U32 causeRxTx;
	struct eth_port *pp = MV_ETH_PRIV(dev);
...
static int mv_eth_load_network_interfaces(void)
{
	...
	for (port = 0; port < mv_eth_ports_num; port++) {
		...
		if (pp->flags & (MV_ETH_F_SWITCH | MV_ETH_F_EXT_SWITCH)) {
			...
			status = mv_eth_switch_config_get(port);
			...
			if (status < 0) {
				...
				if (mv_switch_init(SWITCH_CONNECTED_PORTS_MASK)) {
...
#else                 
		if (pp->flags & (MV_ETH_F_SWITCH_RG)) {
			printk(KERN_ERR "  o Working in Switch RG mode\n");
                }
#endif /* CONFIG_MV_ETH_SWITCH */
		...
		if (!mv_eth_initialized) {
			...
			if (pp->flags & (MV_ETH_F_EXT_SWITCH | MV_ETH_F_SWITCH_RG))
				mv_eth_switch_netdev_first = mv_eth_switch_netdev_last = dev_i;
			else if ((dev_i >= mv_eth_switch_netdev_first) && (dev_i <= mv_eth_switch_netdev_last))
				mv_eth_switch_netdev_first = mv_eth_switch_netdev_last = dev_i + 1;

			pp->dev = mv_net_devs[dev_i];
			dev_i++;
		} else {
			if (pp->flags & (MV_ETH_F_EXT_SWITCH | MV_ETH_F_SWITCH_RG)) {
				mv_eth_netdev_update(mv_eth_switch_netdev_first, pp);
				pp->dev = mv_net_devs[mv_eth_switch_netdev_first];
			} else {
				if ((dev_i >= mv_eth_switch_netdev_first) && (dev_i <= mv_eth_switch_netdev_last))
...
struct net_device *mv_eth_netdev_init(struct eth_port *pp, int mtu, u8 *mac)
{
	...
	if (boardType == MI424WR_I_ID || boardType == MI424WR_J_ID)
...
#elif !defined CONFIG_RG_ATHEROS_ART
		set_bit(MV_ETH_F_SWITCH_RG_BIT, &(pp->flags));
#endif /* CONFIG_MV_INCLUDE_SWITCH */
	}
...
void mv_eth_netdev_print(struct net_device *dev)
{
	...
	printk(KERN_ERR "%s net_device status: dev=%p, pp=%p, state=0x%lx\n\n",
		dev->name, dev, MV_ETH_PRIV(dev), dev->state);
...
void mv_eth_port_status_print(unsigned int port)
{
	...
	printk(KERN_CONT "\n");
	printk(KERN_CONT "CPU:   txq_def   causeRxTx\n");
	{
		int cpu;
		for_each_possible_cpu(cpu)
			printk(KERN_ERR "  %d:      %d      0x%08x\n",
				cpu, pp->txq[cpu], pp->causeRxTx[cpu]);
	}
	printk(KERN_CONT "\n");
...
int mv_eth_check_all_ports_down(void)
{
	...
	for (port = 0; port < mv_eth_ports_num; port++) {
		...
		if (pp == NULL) {
			printk(KERN_ERR "%s: got NULL port (%d)\n", __func__, port);
			return -1;
		}

		if (pp->flags & MV_ETH_F_STARTED) {
			printk(KERN_ERR "%s: port %d is started\n", __func__, port);
			return -1;
		}
	}
