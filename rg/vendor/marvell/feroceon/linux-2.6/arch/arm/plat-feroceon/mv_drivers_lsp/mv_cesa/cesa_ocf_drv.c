...
/* global variables */
#ifndef NO_KERNEL_CRYPTO_FRAMEWORK
...
ocf_check_action(struct cryptop *crp, struct cesa_ocf_data *cesa_ocf_cur_ses) {
	...
	struct cryptodesc *crd;
		...
		if (crd->crd_alg == CRYPTO_GROUP_ENC || crd->crd_alg == cesa_ocf_cur_ses->cipher_alg) {
		        crd->crd_alg = cesa_ocf_cur_ses->cipher_alg;
		...
		/* Authentication */
		else if (crd->crd_alg == CRYPTO_GROUP_AUTH || crd->crd_alg == cesa_ocf_cur_ses->auth_alg) {
		        crd->crd_alg = cesa_ocf_cur_ses->auth_alg;
			/* check that the action is compatible with session */
			if(auth) {
				printk("%s,%d: session mode is not supported.\n", __FILE__, __LINE__);
				return 1;
...
static inline struct cesa_ocf_process* cesa_ocf_alloc(void)
{
	...
	proc = proc_empty;
	proc_empty = ((proc_empty+1) % ((CESA_Q_SIZE + CESA_Q_SIZE_PAD) * MV_CESA_CHANNELS));
	spin_unlock_irqrestore(&cesa_lock, flags);
...
cesa_ocf_process(device_t dev, struct cryptop *crp, int hint)
{
	...
	/* In case request should be split, at least 2 
  	   consecutive slots should be available in CESA fifo */
	if (free_resrc < 3) {
				...
				if (crd->crd_flags & CRD_F_IV_EXPLICIT) {  /* IV from USER */
					...
					ivp = crd->crd_iv;
                                	...
                                	/*
                                 	 ...
                                 	 */
					if (crd->crd_flags & CRD_F_IV_PRESENT_NO_PUT_ASIDE) {
						dprintk("%s,%d: don't copy the IV back to the buffer or to mbuf\n", __FILE__, __LINE__);
						cesa_cmd->ivOffset = crd->crd_inject;
					}
					else if ((crd->crd_flags & CRD_F_IV_PRESENT) == 0) {
...
EXPORT_SYMBOL(cesa_ocf_process);
...
#ifdef NO_KERNEL_CRYPTO_FRAMEWORK
...
EXPORT_SYMBOL(cesa_ocf_newsession);
...
EXPORT_SYMBOL(cesa_ocf_freesession);
...
cesa_ocf_init(void)
{
	...
	cesa_ocf_pool = (struct cesa_ocf_process*)kmalloc((sizeof(struct cesa_ocf_process) * 
					(CESA_Q_SIZE + CESA_Q_SIZE_PAD) * MV_CESA_CHANNELS), GFP_ATOMIC);
	...
	proc_empty = 0;
	memset(cesa_ocf_pool, 0, (sizeof(struct cesa_ocf_process) * 
					(CESA_Q_SIZE + CESA_Q_SIZE_PAD) * MV_CESA_CHANNELS));
#ifndef NO_KERNEL_CRYPTO_FRAMEWORK
...
#define OCF_ID &cesa_ocf_id
	...
	for(chan = 0; chan < MV_CESA_CHANNELS; chan++) {
		...
		if( request_irq( CESA_IRQ(chan), cesa_interrupt_handler,
                             	(IRQF_DISABLED) , irq_str[chan], OCF_ID) < 0) {
