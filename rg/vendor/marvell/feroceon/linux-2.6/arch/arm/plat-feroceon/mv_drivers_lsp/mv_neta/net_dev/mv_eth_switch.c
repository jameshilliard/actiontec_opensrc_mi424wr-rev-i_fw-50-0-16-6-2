...
int mv_eth0_cmdline_config(char *s)
{
	...
	if (mvBoardIsWanSwapEnabled()) {
		net_config_str[MV_ETH_PORT_1] = s;
		net_config_str[MV_ETH_PORT_0] = NULL;
	}
	return 1;
}
...
void mv_eth_switch_swap_config_str(void)
{
	if (mvBoardSwitchConnectedPortGet(0) != -1) {
		net_config_str[MV_ETH_PORT_0] = net_config_str[MV_ETH_PORT_1];
		net_config_str[MV_ETH_PORT_1] = NULL;
	} else if (mvBoardSwitchConnectedPortGet(1) != -1) {
		net_config_str[MV_ETH_PORT_1] = net_config_str[MV_ETH_PORT_0];
		net_config_str[MV_ETH_PORT_0] = NULL;
	}
}
...
int mv_eth_switch_config_get(int port)
{
	char *p_net_config;
	int i = 0;
	...
	for (i = 0; (i < CONFIG_MV_ETH_SWITCH_NETDEV_NUM) && (*p_net_config != '\0'); i++) {
		...
		/* If we have a comma after the closing bracket, then interface definition is done. */
		if (*p_net_config == ',') {
			p_net_config++;
			break;
		}
...
int    mv_eth_switch_start(struct net_device *dev)
{
	...
	if (mv_eth_switch_started == 0)	{
		...
		netif_poll_enable(dev); /* for all UP (devices > 0) on same port */
...
void    mv_eth_switch_status_print(int port)
{
	...
	if (pp->flags & MV_ETH_F_SWITCH) {
		...
		printk(KERN_ERR "mtu=%d, netdev_max=%d, netdev_cfg=%d, first=%d, last=%d, pp->dev->name=%s\n",
			switch_net_config[port].mtu, switch_net_config[port].netdev_max, switch_net_config[port].netdev_cfg,
			mv_eth_switch_netdev_first, mv_eth_switch_netdev_last, pp->dev->name);
