			 ...
			 struct device_attribute *attr, const char *buf, size_t len)
{
	...
	netdev = dev_get_by_name(br_name);
	if (netdev == NULL) {
		err = 1;
		goto bridge_rule_out;
	}
	...
	if (!strcmp(name, "rule_add")) {
		netdev = dev_get_by_name(if_name);
		if (netdev == NULL) {
			err = 1;
			goto bridge_rule_out;
		}
			 ...
			 struct device_attribute *attr, const char *buf, size_t len)
{
	...
	netdev = dev_get_by_name(iif_name);
	if (netdev == NULL) {
		err = 1;
		goto bridge_rule_out;
	}
	...
	if (!strcmp(name, "rule_add")) {
		netdev = dev_get_by_name(oif_name);
		if (netdev == NULL) {
			err = 1;
			goto bridge_rule_out;
		}
			 ...
			 struct device_attribute *attr, const char *buf, size_t len)
{
	...
	br_netdev = dev_get_by_name(bridge_name);
	if_netdev = dev_get_by_name(if_name);
...
{
	int err;
	struct kobject *bridge_kobj;

	/* bridge_kobj = kobject_create_and_add("bridge", nfp_kobj);*/
	bridge_kobj = kobject_add_dir(nfp_kobj, "bridge");
	if (!bridge_kobj) {
		printk(KERN_INFO "could not create bridge kobject \n");
		return -ENOMEM;
	}

	bridge_kobj->kset = nfp_kobj->kset;
