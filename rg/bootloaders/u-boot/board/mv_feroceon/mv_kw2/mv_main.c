...
#if defined(MI424WR_K1_SWITCH)
	extern int mv_switch_init(void);
	mv_switch_init();
#endif
...
int mv_set_power_scheme(void)
{
	...
	/* GE Phy */
	gePhy = MV_FALSE;
	if ((mvBoardIdGet() != MI424WR_I_ID) && (mvBoardIdGet() != MI424WR_J_ID) && 
	    !(ethCompOpt & (ESC_OPT_GEPHY_MAC1 | ESC_OPT_GEPHY_SW_P0 | ESC_OPT_GEPHY_SW_P5))) {
		mvOsOutput("       GE-PHY\n");
		gePhy = MV_TRUE;
	}
